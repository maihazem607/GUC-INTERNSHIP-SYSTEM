...
const MyInternshipsPage: React.FC = () => {
  ...
  const { notification, visible, showNotification, hideNotification } = useNotification({
    periodicNotification: {
      enabled: true,
      message: 'Check your internship reports and evaluations!',
      type: 'info'
    }
  });
  ...
  const handleSubmitReport = () => {
    if (selectedInternship) {
      ...
      setTimeout(() => {
        ...
        // Show success notification with report status
        showNotification({
          message: `Report for ${selectedInternship.title} at ${selectedInternship.company} ${report.finalized ? 'finalized' : 'saved as draft'}.`,
          type: 'success'
        });
      }, 800); // Simulate network delay
    }
  ...
  const handleDownloadPDF = () => {
    if (selectedInternship) {
      ...
      setTimeout(() => {
        ...
        // Generate a filename based on internship details
        const fileName = `${selectedInternship.company.replace(' ', '_')}_${selectedInternship.title.replace(' ', '_')}_Report${report.finalized ? '' : '_DRAFT'}.pdf`;
