...
const internships: Internship[] = [
  {
    ...
    salary: '$250/hr',
    logo: '/logos/amazon.png',
    description: `Join Amazon's design team to create world-class user experiences. You'll work on high-impact projects and collaborate with product managers and engineers to deliver intuitive interfaces for millions of users.`
  },
  {
    id: 2,
    company: 'Google',
...
export default function InternshipListPage() {
  ...
  // Add notification system with periodic notifications enabled
  const { notification, visible, showNotification, hideNotification } = useNotification({
    periodicNotification: {
      enabled: true,
      message: 'Check out new internship opportunities!',
      type: 'info'
    }
  });
  ...
  // Internship cycle notification logic
  useEffect(() => {
    const checkInternshipCycle = () => {
      const currentDate = new Date();
      // Example: Assume internship cycle starts on June 1st, 2025
      const cycleStartDate = new Date(2025, 5, 1); // June 1st, 2025
      const oneWeekBeforeCycle = new Date(cycleStartDate);
      oneWeekBeforeCycle.setDate(cycleStartDate.getDate() - 7); // One week before

      // Check if today is the start of the cycle
      if (
        currentDate.getFullYear() === cycleStartDate.getFullYear() &&
        currentDate.getMonth() === cycleStartDate.getMonth() &&
        currentDate.getDate() === cycleStartDate.getDate()
      ) {
        showNotification({
          message: 'New internship cycle has started! Apply now for exciting opportunities.',
          type: 'info'
        });
      }
      // Check if today is one week before the cycle
      else if (
        currentDate.getFullYear() === oneWeekBeforeCycle.getFullYear() &&
        currentDate.getMonth() === oneWeekBeforeCycle.getMonth() &&
        currentDate.getDate() === oneWeekBeforeCycle.getDate()
      ) {
        showNotification({
          message: 'Internship cycle starts in one week! Prepare your applications.',
          type: 'warning'
        });
      }
    };
    ...
    const interval = setInterval(checkInternshipCycle, 24 * 60 * 60 * 1000); // Every 24 hours
