...
interface InternsListProps {
  interns: Array<any>; // Should use the Intern type from the types.ts file
  onStatusChange: (internId: string, status: 'current' | 'completed', endDate?: Date) => void;
  onEvaluate: (intern: any) => void;
}

// Main component
const InternsList: React.FC<InternsListProps> = ({ interns, onStatusChange, onEvaluate }) => {
  // We'll use the provided interns instead of local state
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm overflow-hidden">
      <table className="w-full border-collapse">
        <tbody>
          {interns.map(intern => (
            <tr key={intern.id} className="hover:bg-blue-50 cursor-pointer border-b border-gray-100">
              {/* Profile Column */}
            ...
            <td className="py-4 px-2 w-32">
              <select
                ...
                value={intern.status}
                onChange={(e) => onStatusChange(intern.id, e.target.value as 'current' | 'completed')}
